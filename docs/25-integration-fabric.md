...
The overriding goal is to preserve the untrusted-worker safety invariants *by construction*. FloorLingo
...
**~90% a faithful clone of seams FloorLingo already ships**:
...
flowchart LR
    ...
    subgraph Core["FloorLingo host (core)"]
...
- **Authentication inversion.** A provider webhook cannot carry an FloorLingo API key, so ingress is public to
