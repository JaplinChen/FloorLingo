...
### Adapter
An interface implementation that provides a specific capability. In FloorLingo, adapters are used for:
...
### API Key
Authentication token to access the FloorLingo API. Sent via the `X-API-Key` header.
...
### Baileys
Node.js library for WhatsApp Web that uses WebSocket directly without a browser (no Chromium required). Available as a selectable engine in FloorLingo via `ENGINE_TYPE=baileys`.
...
### Chrome/Chromium
Browser used by Puppeteer to run WhatsApp Web. FloorLingo uses headless Chromium.
...
### Dashboard
Web interface to manage FloorLingo without using the API directly. Built with React + shadcn/ui.

### Dead Letter Queue (DLQ)
Queue that stores messages that failed after all retry attempts. Used for debugging and manual retry.

### Docker
Containerization platform for packaging and deploying applications. FloorLingo is distributed as a Docker image.

## E

### Engine
Component that handles communication with WhatsApp Web. FloorLingo supports pluggable engines selected via the `ENGINE_TYPE` environment variable: `whatsapp-web.js` (default, Chromium/Puppeteer-based) or `baileys` (browser-free, WebSocket-based).
...
### GHCR
GitHub Container Registry - registry for storing Docker images. The FloorLingo image is available at `ghcr.io/rmyndharis/openwa`.
...
### JID (Jabber ID)
WhatsApp's id format, inherited from XMPP; the user-facing "Chat ID" is a JID. The same entity can be addressed in more than one dialect: `<phone>@c.us` (whatsapp-web.js, and FloorLingo's neutral form), `<phone>@s.whatsapp.net` (Baileys' raw form for the same user), `<id>@g.us` (a group), or `<lid>@lid` (a LID, a privacy id). FloorLingo normalizes engine ids to a single neutral dialect at the engine boundary - see *System Architecture > WhatsApp Identity Contract*.
...
### Multi-session
Ability to run multiple WhatsApp sessions within a single FloorLingo instance.

## N

### NestJS
Node.js framework for building server-side applications. The FloorLingo backend is built with NestJS.

### Node.js
JavaScript runtime used to run FloorLingo. Recommended version: Node.js 22 LTS.

## O

### ORM (Object-Relational Mapping)
Library that maps objects in code to database tables. FloorLingo uses TypeORM.
...
### Plugin
Extension that can be added to FloorLingo to add functionality without modifying the core codebase.
...
### REST API
Architectural style for APIs used by FloorLingo. Uses HTTP methods (GET, POST, PUT, DELETE).
...
AWS object storage service. FloorLingo supports S3-compatible storage for media files.
...
### Swagger
API documentation tool. FloorLingo provides Swagger UI at `/api/docs`.
...
### TypeScript
Typed superset of JavaScript used for FloorLingo development.
...
### Volume (Docker)
Persistent storage for Docker containers. FloorLingo data is stored in volumes.

## W

### WAHA
WhatsApp HTTP API - a similar project that inspired FloorLingo. FloorLingo is built as an open-source alternative.
...
### whatsapp-web.js
Default engine library used by FloorLingo to interact with WhatsApp Web. Uses Puppeteer to control a headless Chromium browser. Selected via `ENGINE_TYPE=whatsapp-web.js` (or by omitting the env var).
