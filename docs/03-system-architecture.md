...
FloorLingo is designed with a **Pluggable Architecture** that allows infrastructure components to be swapped without changing application code. This enables flexible deployments ranging from minimal single-session bots to larger single-node, multi-session installs.

> **Note — single-instance:** the live WhatsApp engine layer is stateful and held in-process
> (an in-memory `Map` in `SessionService`). FloorLingo currently runs as **one API instance per
...
FloorLingo does **not** use a dynamic `AdaptersModule` or string DI tokens. `AppModule`
...
> [!IMPORTANT]
> Engine abstraction is critical to mitigate **R001: WhatsApp Protocol Changes** in Risk Management. FloorLingo ships two production-ready engines selectable via `ENGINE_TYPE`: `whatsapp-web.js` (default, Chromium/Puppeteer-based) and `baileys` (browser-free, WebSocket/Noise protocol). With the abstraction layer, adding further engines requires no changes to application code.
...
FloorLingo uses the adapter pattern for infrastructure components that can be swapped per deployment needs. This allows users with limited resources to run FloorLingo without heavyweight external dependencies.

### Adapter Overview

```mermaid
flowchart TB
    subgraph Core["FloorLingo Core"]
...
FloorLingo supports SQLite for lightweight deployments and PostgreSQL for high-volume production.
...
> **Note:** FloorLingo does not currently apply SQLite-specific concurrency hardening. There is **no**
...
FloorLingo provides several deployment profiles for different needs:
...
> FloorLingo runs as a single API instance per session-data volume; there is no cluster-mode flag.
...
FloorLingo does not auto-detect a profile at runtime; pick one by available resources and expected load:
