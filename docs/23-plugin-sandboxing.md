...
FloorLingo runs **untrusted plugins** (anything loaded from the plugins directory) in an isolated worker
...
as the same uid as FloorLingo. The sandbox therefore does **not**, by itself, stop a malicious plugin
from reading files the FloorLingo process can read or making outbound network connections. It protects
...
- **Run FloorLingo in its container** (the shipped Docker image already runs read-only rootfs, non-root,
