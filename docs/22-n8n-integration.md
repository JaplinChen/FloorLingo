...
FloorLingo provides official n8n community nodes for integrating WhatsApp automation into n8n workflows. This enables users to build powerful automations combining WhatsApp messaging with hundreds of other services available in n8n.
...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   n8n Workflow  │────▶│  OpenWA Node    │────▶│  FloorLingo API │
...
│   (triggered)   │     │  (listens)      │     │  from FloorLingo│
└─────────────────┘     └─────────────────┘     └─────────────────┘
```
...
Execute operations on your FloorLingo server.

#### Credentials Setup

| Field      | Description                      | Example                  |
| ---------- | -------------------------------- | ------------------------ |
| Server URL | FloorLingo server URL (without /api) | `https://wa.example.com` |
| API Key    | API key from FloorLingo dashboard    | `owa_xxxxxxxx...`        |
...
1. When workflow is activated, the trigger creates a webhook in FloorLingo
2. FloorLingo sends events to n8n's webhook URL
...
1. Verify FloorLingo server is running
2. Check API key is correct
3. Ensure server URL doesn't have trailing slash
4. Verify network connectivity between n8n and FloorLingo

### Trigger Not Receiving Events

1. Check webhook was created in FloorLingo dashboard
2. Verify n8n webhook URL is accessible from FloorLingo server
...
- [FloorLingo API Specification](./06-api-specification.md)
