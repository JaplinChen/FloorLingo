...
This collection gives a runnable cURL for every FloorLingo REST endpoint. The examples assume two environment variables — set them once and reuse them:
...
curl -X POST "$BASE/api/sessions/$SESSION_ID/status/send-text" \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "text": "Hello from FloorLingo!", "backgroundColor": "#25D366", "font": 2 }'
```
