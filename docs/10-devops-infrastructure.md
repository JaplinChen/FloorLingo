...
> [!IMPORTANT]
> **Keep `replicas: 1`.** FloorLingo is a single-process application: live engine state lives in an
...
> **Design sketch, not a supported topology.** FloorLingo is single-process with in-memory engine state,
> so the multi-`FloorLingo` fan-out below would corrupt WhatsApp auth across replicas. It is retained only
...
These rules use the metric names FloorLingo actually exports (`openwa_*`). The memory rule below uses a
    ...
    rules:
      ...
      - alert: ServiceDown
        ...
        labels:
          severity: critical
        annotations:
          summary: "FloorLingo service is down"
          description: "The FloorLingo application is not responding"
      ...
      - alert: HighProcessMemory
        ...
        labels:
          severity: warning
        annotations:
          summary: "High FloorLingo process memory"
          description: "RSS is {{ $value | humanize1024 }}B"

      # Host memory pressure — EXTERNAL (node-exporter), not exported by FloorLingo
...
The metrics surface is small, so FloorLingo emits Prometheus text exposition format (v0.0.4) **by hand** —
...
```json
// monitoring/grafana/dashboards/openwa.json — panels use the openwa_* metrics FloorLingo exports
{
  "title": "FloorLingo Dashboard",
  "uid": "openwa-main",
  "panels": [
    {
...
These are the metrics FloorLingo actually exports at `GET /api/metrics`:
...
> FloorLingo does **not** expose request-rate, latency-histogram, webhook, queue, or Node default
...
FloorLingo scales **vertically** — add CPU/RAM to a single instance. The table below is **unbenchmarked
...
**Not currently supported.** FloorLingo is a single-process application with in-memory engine state, so
