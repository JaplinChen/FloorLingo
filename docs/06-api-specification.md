...
FloorLingo seeds an initial admin key on first run (printed to the startup log and written to `data/.api-key`, or `/app/data/.api-key` in Docker). Use it to mint scoped, lower-privilege keys for integrations. Full key creation, listing, rotation, and revocation are documented under the auth resource in **§6.4.9 (API Keys)**.

## 6.2 Response Format

> **FloorLingo returns the raw handler payload directly — there is NO `{ success, data, meta }` envelope.** A resource route returns the resource object as-is; a list route returns a **bare JSON array**. Read fields directly (`response.id`, not `response.data.id`).
...
FloorLingo uses **two** timestamp representations — be careful which a field is:
...
```json
{ "chatId": "628123456789@c.us", "text": "Hello from FloorLingo!" }
```
...
```json
{ "text": "Hello from FloorLingo!", "recipients": ["6281234567890@c.us"], "backgroundColor": "#25D366", "font": 2 }
```
...
Prometheus exposition scrape of FloorLingo process + session + message metrics; gated by a `METRICS_TOKEN` bearer (disabled when the token is unset).
...
```
# HELP openwa_up 1 if the FloorLingo process is running
...
These are the events FloorLingo actually emits. A webhook is registered with an `events` list; an event is delivered to a webhook when its `events` array includes the event name or `"*"`.
...
**Design your handler to be idempotent**, keyed on the `X-OpenWA-Idempotency-Key` header (see below). As a server-side safety net, FloorLingo de-duplicates inbound `message.received` before dispatch (a re-fired event for an already-persisted message is dropped), so one webhook normally sees each inbound message once — but this is best-effort defense-in-depth and does not remove the need for consumer-side idempotency.
